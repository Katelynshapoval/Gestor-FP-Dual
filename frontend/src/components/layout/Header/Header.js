...
function Header({ sidebarCollapsed = false, onSidebarToggle }) {
  ...
  const Brand = ({ compact = false, collapsed = false }) => (
    ...
    >
      <img
        src="logo.png"
        alt="Salesianos"
        className={compact || collapsed ? "h-8 w-8" : "h-9 w-9"}
      />
  ...
  return (
    <>
      ...
      >
        ...
        {location !== "/login" && (
          ...
          >
            ...
            {sidebarCollapsed && (
              <span className="nav-tooltip">{sessionLabel}</span>
            )}
          </button>
        )}
      </aside>
      ...
      <div
        className={`mobile-drawer ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        <div className="flex flex-1 flex-col gap-5 px-5 py-5">
          ...
          {location !== "/login" && (
            <button
              onClick={handleSessionAction}
              className="nav-session-button mt-auto"
            >
              <SessionIcon className="nav-icon" />
              <span className="nav-label">{sessionLabel}</span>
            </button>
